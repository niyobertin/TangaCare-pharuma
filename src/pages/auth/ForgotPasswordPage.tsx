            ...
            <div className="max-w-md w-full animate-in zoom-in duration-500 relative z-10">
                <form onSubmit={handleSubmit(onSubmit)} className="glass-card shadow-2xl p-10 space-y-8 bg-white/95 dark:bg-slate-900/95 border border-white/20 rounded-2xl">
                    <div className="text-center space-y-2">
                        <h2 className="text-3xl font-black text-healthcare-dark tracking-tight">Forgot Password?</h2>
                        <p className="text-slate-400 font-bold text-xs uppercase tracking-wider">no worries, we'll send you reset instructions.</p>
                    </div>
                    ...
                    <div className="space-y-4">
                        <div className="space-y-1.5">
                            ...
                            {errors.email && <p className="text-[10px] font-normal text-red-500 ml-1">{errors.email.message}</p>}
                        </div>
                    </div>
                    ...
                    >
                        {loading ? <div className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin"></div> : 'Send OTP'}
                    </button>
