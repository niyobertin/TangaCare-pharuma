                ...
                <div className="flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-slate-500 text-sm mt-1">
                            Manage medication recalls, notify patients, and track recovered inventory.
                        </p>
                    </div>
                    <button
                        ...
                        <table className="w-full text-left text-sm whitespace-nowrap">
                            <thead className="bg-slate-50 dark:bg-slate-800/50">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 font-bold text-slate-500">Medicine & Batch</th>
                                    <th className="px-6 py-4 font-bold text-slate-500">Reason</th>
                                    <th className="px-6 py-4 font-bold text-slate-500">Affected Sales</th>
                                    <th className="px-6 py-4 font-bold text-slate-500">Recovery Status</th>
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {recalls.length > 0 ? (
                                    recalls.map((recall) => (
                                        <tr key={recall.id} className="hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="font-bold text-slate-900 dark:text-white">{recall.medicine?.name}</div>
                                                <div className="text-xs text-slate-400">Batch: {recall.batch?.batch_number}</div>
                                            </td>
                                            <td className="px-6 py-4">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="text-slate-900 dark:text-white font-bold">{recall.affected_sales_count}</div>
                                                <div className="text-[10px] text-slate-400">{recall.affected_quantity} units total</div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="w-24 h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full mt-1 overflow-hidden">
                                                    <div
                                                        className="h-full bg-rose-500"
                                                        style={{ width: `${Math.min(100, (recall.recovered_quantity / recall.affected_quantity) * 100 || 0)}%` }}
                                                    ></div>
                                                </div>
                                                <div className="text-[10px] text-slate-500 mt-1">
                                                    {recall.recovered_quantity} / {recall.affected_quantity} recovered
                                                </div>
                                            </td>
    ...
    return (
        <div className="max-w-2xl mx-auto p-6 space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 space-y-4">
                {step === 1 ? (
                    <>
                        <label className="block text-sm font-bold text-slate-700">Search Batch to Recall</label>
                        ...
                        <div className="max-h-60 overflow-y-auto divide-y">
                            {batches.map(b => (
                                <div
                                    key={b.id}
                                    onClick={() => { setSelectedBatch(b); setStep(2); }}
                                ...
                                >
                                    <div>
                                        ...
                                        <div className="text-xs text-slate-500">Batch: {b.batch_number} • Exp: {b.expiry_date}</div>
                                    </div>
                    ...
                    <>
                        <div className="p-3 bg-rose-50 border border-rose-100 rounded-lg">
                            <div className="text-xs text-rose-500 font-bold uppercase">Selected for Recall</div>
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-bold mb-1">Recall Reason</label>
                                <select
                                    value={reason}
                        ...
                        <div className="flex gap-3 pt-4">
                            <button onClick={onBack} className="flex-1 py-2 border rounded-lg font-bold">Cancel</button>
                            <button
        ...
        <div className="p-6 space-y-6">
            <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                    ...
                    <div className="text-sm text-slate-500">{recall.medicine?.name} • Batch {recall.batch?.batch_number}</div>
                </div>
                    ...
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                ...
                                <div className="font-bold capitalize">{recall.reason.replace(/_/g, ' ')}</div>
                            </div>
                            <div>
                                <div className="text-slate-400">Initiated At</div>
                                <div className="font-bold">{format(new Date(recall.initiated_at), 'PPP')}</div>
                            </div>
                    ...
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-xl">
                                <div className="text-xs text-slate-400 uppercase font-black">Total Affected</div>
                                <div className="text-2xl font-black">{recall.affected_quantity}</div>
                            </div>
                            <div className="p-4 bg-emerald-50 dark:bg-emerald-900/10 rounded-xl">
                                <div className="text-xs text-emerald-600 uppercase font-black">Recovered</div>
                                <div className="text-2xl font-black text-emerald-600">{recall.recovered_quantity}</div>
                            </div>
                            <div className="p-4 bg-amber-50 dark:bg-amber-900/10 rounded-xl">
                                <div className="text-xs text-amber-600 uppercase font-black">Remaining Out</div>
                                <div className="text-2xl font-black text-amber-600">{recall.affected_quantity - recall.recovered_quantity}</div>
                            </div>
                        </div>
                    </div>
                </div>
...
function StatusBadge({ status }: { status: RecallStatus }) {
    switch (status) {
        case RecallStatus.INITIATED:
            return <span className="px-2 py-0.5 rounded-full bg-blue-50 text-blue-600 border border-blue-100">Initiated</span>;
        case RecallStatus.IN_PROGRESS:
            return <span className="px-2 py-0.5 rounded-full bg-amber-50 text-amber-600 border border-amber-100">In Progress</span>;
        case RecallStatus.COMPLETED:
            return <span className="px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-600 border border-emerald-100">Completed</span>;
        default:
            return <span className="px-2 py-0.5 rounded-full bg-slate-50 text-slate-500 border border-slate-100">{status}</span>;
    }
}
