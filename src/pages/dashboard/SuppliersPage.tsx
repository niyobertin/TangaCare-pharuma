...
export function SuppliersPage() {
    ...
    const stats = [
        { label: 'Total Partners', value: totalItems, icon: Building2, color: 'text-blue-500', bg: 'bg-blue-50' },
        { label: 'Active Orders', value: 12, icon: TrendingUp, color: 'text-teal-500', bg: 'bg-teal-50' },
        { label: 'Avg lead Time', value: '3.5 Days', icon: History, color: 'text-indigo-500', bg: 'bg-indigo-50' },
    ];
            ...
            <div className="p-5 space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-700">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div className="space-y-1">
                        <h2 className="text-2xl font-black text-healthcare-dark tracking-tight">Manufacturers & Suppliers</h2>
                        <p className="text-slate-500 font-bold text-xs uppercase tracking-wider">Supply Chain & Vendor Management</p>
                    </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-slate-50 dark:bg-slate-800/50">
                                    <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-400 tracking-widest">Supplier Info</th>
                                    <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-400 tracking-widest">Contact Details</th>
                                    <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-400 tracking-widest">Address</th>
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {loading ? (
                                    ...
                                    filteredSuppliers.map((supplier) => (
                                        <tr key={supplier.id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col">
                                                    <span className="font-black text-healthcare-dark text-sm leading-tight">{supplier.name}</span>
                                                    <span className="text-[10px] text-slate-400 font-bold uppercase mt-1">Ref: SUP-{supplier.id.toString().padStart(3, '0')}</span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white dark:bg-slate-900 p-4 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-sm">
                    <div className="text-[11px] font-black uppercase text-slate-400 tracking-widest">
                        Showing <span className="text-healthcare-dark">{totalItems === 0 ? 0 : (page - 1) * limit + 1}</span> to <span className="text-healthcare-dark">{Math.min(page * limit, totalItems)}</span> of <span className="text-healthcare-dark">{totalItems}</span> Suppliers
                    </div>
