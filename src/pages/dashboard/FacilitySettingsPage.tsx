...
export function FacilitySettingsPage() {
    ...
    const handleAssignAdmin = async (user: User) => {
        if (!facility) return;
        if (!confirm(`Are you sure you want to assign ${user.first_name} as admin for this facility?`)) return;
    ...
    return (
        <div className="p-6 max-w-5xl xl:max-w-6xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    <h1 className="text-2xl font-black text-healthcare-dark">Facility Settings</h1>
                ...
                <div className="ml-auto flex items-center gap-3">
                    <div className={cn(
                        ...
                        facility?.status === 'Active' ? "bg-emerald-50 text-emerald-600" : "bg-slate-100 text-slate-500"
                    )}>
                        <div className={cn("w-2 h-2 rounded-full", facility?.status === 'Active' ? "bg-emerald-500" : "bg-slate-400")} />
            ...
            <div className="max-w-4xl">
                <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-800">
                        ...
                        <div className="space-y-8">
                            <div className="space-y-6">
                                <div className="border-b border-slate-100 dark:border-slate-800 pb-4 mb-6">
                                    <h3 className="text-lg font-black text-healthcare-dark">Basic Information</h3>
                                    <p className="text-slate-400 text-xs">Primary facility details and contact info</p>
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-xs font-black uppercase text-slate-400">Facility Name</label>
                                        <input
                                            type="text"
                                            value={formData.name}
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-xs font-black uppercase text-slate-400">Facility Type</label>
                                        <select
                                            value={formData.type}
                            ...
                            <div className="pt-8 mt-4 border-t border-slate-200 dark:border-slate-800">
                                ...
                                <div className="bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30 rounded-xl p-4 flex items-center justify-between">
                                    <div>
                                        <p className="font-bold text-red-700 dark:text-red-400">Delete this facility</p>
                                        <p className="text-xs text-red-600/70 dark:text-red-400/70 mt-1">Once deleted, this action cannot be undone.</p>
                                    </div>
                                    <button
                    ...
                    {activeTab === 'config' && (
                        <div className="space-y-6">
                            <div className="border-b border-slate-100 dark:border-slate-800 pb-4 mb-6">
                                <h3 className="text-lg font-black text-healthcare-dark">System Configuration</h3>
                                <p className="text-slate-400 text-xs">Manage operational rules and thresholds</p>
                            </div>

                            <div className="space-y-4">
                                <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700">
                                    <div>
                                        <h4 className="font-bold text-healthcare-dark text-sm">Enable Departments</h4>
                                        <p className="text-xs text-slate-500 mt-1">Allow facility to be split into multiple departments</p>
                                    </div>
                                ...
                                <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700">
                                    <div>
                                        <h4 className="font-bold text-healthcare-dark text-sm">Controlled Drug Rules</h4>
                                        <p className="text-xs text-slate-500 mt-1">Enforce strict logging for controlled substances</p>
                                    </div>
                                    <label className="relative inline-flex items-center cursor-pointer">
                                        <input
                                            type="checkbox"
                                ...
                                <div className="grid grid-cols-2 gap-4 mt-4">
                                    <div className="space-y-2">
                                        <label className="text-xs font-black uppercase text-slate-400">Min Stock Threshold (%)</label>
                                        <input
                                            type="number"
                                            value={formData.min_stock_threshold_percentage}
                                            onChange={(e) => setFormData(p => ({ ...p, min_stock_threshold_percentage: Number(e.target.value) }))}
                                            className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-800 rounded-xl focus:outline-none focus:border-healthcare-primary font-bold text-healthcare-dark"
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-xs font-black uppercase text-slate-400">Expiry Alert (Days)</label>
                                        <input
                                            type="number"
                                            value={formData.expiry_alert_days}
                                            onChange={(e) => setFormData(p => ({ ...p, expiry_alert_days: Number(e.target.value) }))}
                                            className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-800 rounded-xl focus:outline-none focus:border-healthcare-primary font-bold text-healthcare-dark"
                                        />
                                    </div>
                                </div>
                    ...
                    {activeTab === 'admin' && (
                        <div className="space-y-6">
                            <div className="border-b border-slate-100 dark:border-slate-800 pb-4 mb-6">
                                <h3 className="text-lg font-black text-healthcare-dark">Facility Administrator</h3>
                                <p className="text-slate-400 text-xs">Assign or change the primary admin for this facility</p>
                            </div>
                            ...
                            <div className="bg-blue-50 dark:bg-blue-900/10 p-4 rounded-xl flex items-center gap-4 mb-6">
                                ...
                                <div>
                                    <p className="text-xs font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wide">Current Admin</p>
                                    <p className="text-base font-black text-healthcare-dark">
                                        {facility?.facility_admin ? `${facility.facility_admin.first_name || ''} ${facility.facility_admin.last_name || ''}`.trim() || 'Admin' : (facility?.admin_name || 'No Admin Assigned')}
                                    </p>
                                </div>
                            </div>

                            <div className="space-y-4">
                                <label className="text-xs font-black uppercase text-slate-400">Assign New Admin</label>
                                <input
                                    type="text"
                                    value={adminQuery}
                                        ...
                                        {searchingUsers ? (
                                            ...
                                            <div className="divide-y divide-slate-100 dark:divide-slate-800">
                                                {users.map(user => (
                                                    ...
                                                    >
                                                        <div>
                                                            <div className="font-bold text-sm text-healthcare-dark">{user.first_name} {user.last_name}</div>
