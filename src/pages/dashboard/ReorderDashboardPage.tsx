...
export function ReorderDashboardPage() {
    ...
    const handleCreateDraftPOs = async () => {
        ...
        setCreatingPO(true);
        try {
            const response = await pharmacyService.createDraftPOsFromSuggestions(effectiveFacilityId);
            setPoCreated(`Successfully created ${(response as any).count || 'new'} draft Purchase Orders.`);
            loadSuggestions();
        } catch (error) {
            ...
            <div className="p-6 space-y-6 animate-in fade-in duration-500">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-black text-healthcare-dark dark:text-white flex items-center gap-2">
                            <Zap className="text-amber-500" fill="currentColor" /> Predictive Reordering
                        </h1>
                        <p className="text-slate-500 text-sm mt-1">
                            Intelligent procurement suggestions based on consumption patterns and stock levels.
                        </p>
                    </div>
                    ...
                    >
                        {creatingPO ? <Loader2 className="animate-spin" size={18} /> : <FilePlus size={18} />}
                ...
                {poCreated && (
                    <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-xl flex items-center gap-3 text-emerald-800 animate-in slide-in-from-top duration-300">
                        ...
                        <button onClick={() => setPoCreated(null)} className="ml-auto text-emerald-400 hover:text-emerald-600">×</button>
                    </div>
                )}
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden">
                    <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50">
                        <h2 className="font-black text-slate-800 dark:text-white flex items-center gap-2 italic">
                            <ShoppingCart className="text-healthcare-primary" size={20} /> Suggested Reorder List
                        </h2>
                    </div>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left text-sm">
                                <thead className="bg-slate-50 dark:bg-slate-800/80">
                                    <tr>
                                        <th className="px-6 py-4 font-bold text-slate-500 uppercase tracking-widest text-[10px]">Medicine</th>
                                        <th className="px-6 py-4 font-bold text-slate-500 uppercase tracking-widest text-[10px] text-right">Current Stock</th>
                                        <th className="px-6 py-4 font-bold text-slate-500 uppercase tracking-widest text-[10px] text-right">Target Level</th>
                                        <th className="px-6 py-4 font-bold text-slate-500 uppercase tracking-widest text-[10px] text-right">Order Suggestion</th>
                                        <th className="px-6 py-4 font-bold text-slate-500 uppercase tracking-widest text-[10px]">Urgency</th>
                                        ...
                                        suggestions.map((s, idx) => (
                                            <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                                                <td className="px-6 py-4">
                                                    <div className="font-bold text-slate-900 dark:text-white">{s.medicine_name}</div>
                                                    <div className="text-[10px] text-slate-400 font-mono">ID: {s.medicine_id}</div>
                                                </td>
                                                <td className="px-6 py-4 text-right font-medium">
                                                    <span className={s.current_quantity <= s.reorder_point ? 'text-rose-500 font-black' : 'text-slate-700'}>
                                                        {s.current_quantity}
                                                    </span>
                                                </td>
                                                ...
                                                <td className="px-6 py-4">
                                                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest ${s.urgency === 'high' ? 'bg-rose-50 text-rose-600 border border-rose-100' :
                                                        s.urgency === 'medium' ? 'bg-amber-50 text-amber-600 border border-amber-100' :
                                        ...
                                        <tr>
                                            <td colSpan={6} className="px-6 py-24 text-center">
                                                <div className="flex flex-col items-center gap-3">
                                                    ...
                                                    <div className="text-slate-400 font-medium">No reorder suggestions found. Your inventory is optimal!</div>
                                                </div>
                                            </td>
                                        </tr>
                                    )}
...
function StatCard({ title, value, color, icon, subtitle }: { title: string, value: number, color: string, icon: React.ReactNode, subtitle: string }) {
    return (
