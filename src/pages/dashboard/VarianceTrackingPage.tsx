...
export function VarianceTrackingPage() {
    ...
    const handleApprove = async (id: number) => {
        if (!window.confirm('Are you sure you want to approve this variance? This will adjust stock levels.')) return;
        try {
                        ...
                        <table className="w-full text-left text-sm whitespace-nowrap">
                            <thead className="bg-slate-50 dark:bg-slate-800/50">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 font-bold text-slate-500 uppercase tracking-wider">Medicine</th>
                                    <th className="px-6 py-4 font-bold text-slate-500 uppercase tracking-wider">Batch</th>
                                    <th className="px-6 py-4 font-bold text-slate-500 uppercase tracking-wider text-right">System vs Physical</th>
                                    ...
                                    variances.map((v) => (
                                        <tr key={v.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="text-slate-500 text-xs">Sys: {v.system_quantity}</div>
                                                <div className="font-bold text-slate-900 dark:text-white">Phy: {v.physical_quantity}</div>
                                            </td>
                                            <td className="px-6 py-4 text-right">
                                                <div className={`inline-flex items-center gap-1 font-black ${v.variance_quantity > 0 ? 'text-emerald-500' :
                                                    v.variance_quantity < 0 ? 'text-rose-500' : 'text-slate-400'
                                                    }`}>
                                                    {v.variance_quantity > 0 ? <ArrowUpRight size={14} /> :
                                                        v.variance_quantity < 0 ? <ArrowDownRight size={14} /> : null}
                                                    {v.variance_quantity > 0 ? '+' : ''}{v.variance_quantity}
                                                </div>
                                                {v.variance_value && (
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                {v.status === VarianceStatus.PENDING ? (
                                                    ...
                                                    <span className="text-xs text-slate-400 italic">
                                                        Processed by {v.approved_by?.first_name || 'System'}
                                                    </span>
                                                )}
                                            </td>
                                        </tr>
...
function StatusBadge({ status }: { status: VarianceStatus }) {
    switch (status) {
        case VarianceStatus.PENDING:
            return <span className="px-2 py-0.5 rounded-full bg-amber-50 text-amber-600 border border-amber-100 text-[10px] font-black uppercase tracking-wider">Pending</span>;
        case VarianceStatus.APPROVED:
            return <span className="px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-600 border border-emerald-100 text-[10px] font-black uppercase tracking-wider">Approved</span>;
        case VarianceStatus.REJECTED:
            return <span className="px-2 py-0.5 rounded-full bg-rose-50 text-rose-600 border border-rose-100 text-[10px] font-black uppercase tracking-wider">Rejected</span>;
    }
}
