...
export function InventoryPage() {
    ...
    const categories = ['All Categories', 'Antibiotics', 'Pain Relief', 'Anti-Diabetic', 'Supplements', 'Cardiovascular'];

    return (
        <ProtectedRoute allowedRoles={['Admin', 'Pharmacist', 'Super Admin', 'ADMIN', 'PHARMACIST', 'SUPER_ADMIN', 'SUPER_ADMIN']}>
            <div className="p-5 space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-700">
                {/* Header Section */}
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h2 className="text-xl font-black text-healthcare-dark tracking-tight">Medicine Inventory</h2>
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-[13px] font-medium">
                                {loading ? (
                                    ...
                                    medicines.map((med) => (
                                        <tr key={med.id} className="group hover:bg-teal-50/30 dark:hover:bg-teal-900/10 transition-all">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="flex flex-col">
                                                        <span className="font-black text-healthcare-dark text-[14px] leading-tight">{med.name}</span>
                                                        <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase mt-0.5 tracking-tight">{med.code} • {med.strength}</span>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col gap-1.5 min-w-[120px]">
                                                    <div className="flex justify-between items-center text-[11px] font-black">
                                                        <span className={cn(
                                                            (med.stock_quantity || 0) <= 20 ? "text-red-500" : "text-slate-600 dark:text-slate-300"
                                                        )}>
                                                            {med.stock_quantity || 0} Units
                                                        </span>
                                                        <span className="text-slate-400 uppercase tracking-tighter">Min: 20</span>
                                                    </div>
                                                    <div className="w-full h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden border border-slate-200 dark:border-slate-700">
                                                        <div
                                                            className={cn(
                                                                ...
                                                                (med.stock_quantity || 0) <= 20 ? "bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.4)]" : "bg-healthcare-primary"
                                                            )}
                                                            style={{ width: `${Math.min(((med.stock_quantity || 0) / 100) * 100, 100)}%` }}
                                                        ></div>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={cn(
                                                    ...
                                                    <div className={cn(
                                                        ...
                                                        (med.stock_quantity || 0) === 0 ? "bg-red-500 animate-pulse" : (med.stock_quantity || 0) <= 20 ? "bg-amber-500" : "bg-emerald-500"
                                                    )}></div>
                                                    {(med.stock_quantity || 0) === 0 ? 'Out of Stock' : (med.stock_quantity || 0) <= 20 ? 'Low Stock' : 'In Stock'}
                                                </span>
                                            </td>
                                    ...
                                    <tr>
                                        <td colSpan={6} className="px-6 py-10 text-center">
                                            <div className="flex flex-col items-center gap-2">
                                                ...
                                                <span className="text-slate-500 font-bold italic">No medicines found matching your criteria</span>
                                            </div>
                                        </td>
                                    </tr>
                                )}
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white dark:bg-slate-900 p-4 border-2 border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm">
                    <div className="text-[11px] font-black uppercase text-slate-400 tracking-widest">
                        Showing <span className="text-healthcare-dark">{(page - 1) * limit + 1}</span> to <span className="text-healthcare-dark">{Math.min(page * limit, totalItems)}</span> of <span className="text-healthcare-dark">{totalItems}</span> Medicines
                    </div>
