...
import {
    Search, Plus, MapPin, Phone, Mail, Edit, Trash2,
    Building, Globe, Briefcase, FileText, X
} from 'lucide-react';
        ...
        <ProtectedRoute allowedRoles={['admin', 'super_admin', 'store_manager', 'facility_admin']}>
            <div className="p-6 space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h1 className="text-2xl font-black text-healthcare-dark">Supplier Management</h1>
                        <p className="text-slate-500 text-sm mt-1">Manage pharmaceutical suppliers and vendors</p>
                    </div>
                    <button
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {loading ? (
                                    <tr><td colSpan={4} className="px-6 py-8"><TableSkeleton rows={4} columns={4} /></td></tr>
                                ) : filteredSuppliers.length > 0 ? (
                                    filteredSuppliers.map(supplier => (
                                        <tr key={supplier.id} className="group hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <p className="font-bold text-healthcare-dark">{supplier.name}</p>
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={`px-2.5 py-1 rounded-md text-xs font-bold uppercase tracking-wide border ${supplier.is_active
            ...
            {showModal && (
                <SupplierModal
                    ...
                    onSuccess={() => { fetchSuppliers(); setShowModal(false); setEditingSupplier(null); }}
                />
            )}
        </ProtectedRoute>
    );
...
function SupplierModal({ supplier, onClose, onSuccess }: { supplier: Supplier | null, onClose: () => void, onSuccess: () => void }) {
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input {...register('name')} className="input" placeholder="Supplier Name" />
                            {errors.name && <p className="error">{errors.name.message}</p>}
                        </div>
                        <div>
                            <label className="label">Contact Person</label>
                            <input {...register('contact_person')} className="input" placeholder="Manager Name" />
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input {...register('email')} className="input" placeholder="supplier@example.com" />
                            {errors.email && <p className="error">{errors.email.message}</p>}
                        </div>
                    </div>
                    <div>
                        <label className="label">Address</label>
                        <textarea {...register('address')} className="input" rows={2} placeholder="Physical address" />
                        {errors.address && <p className="error">{errors.address.message}</p>}
                    </div>
                    <div>
                        <label className="label">Tax ID (TIN)</label>
                        <input {...register('tax_id')} className="input" placeholder="Tax Identification Number" />
                    </div>
