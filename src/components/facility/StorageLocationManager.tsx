...
import { TemperatureType, type StorageLocation, type CreateStorageLocationDto } from '../../types/pharmacy';
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h3 className="text-lg font-black text-slate-800 dark:text-white">Storage Locations</h3>
                    <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Manage shelves, racks, and cold storage units</p>
                </div>
                <button
                    onClick={() => {
                        setEditingLocation(null);
                ...
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden animate-in zoom-in-95 duration-300">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Temperature Requirement</label>
                                <select
                                    value={formData.temperature_type}
                                    onChange={(e) => setFormData({ ...formData, temperature_type: e.target.value as TemperatureType })}
                                    className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800/50 border-2 border-transparent focus:border-healthcare-primary/20 focus:bg-white dark:focus:bg-slate-800 rounded-xl text-sm font-bold outline-none transition-all"
                                >
                                    <option value={TemperatureType.ROOM_TEMP}>Room Temperature</option>
                                    <option value={TemperatureType.COLD}>Cold Storage (2-8°C)</option>
                                    <option value={TemperatureType.FROZEN}>Frozen Storage (&lt;0°C)</option>
                                </select>
                            </div>
                            ...
                            <div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                                ...
                                />
                                <label htmlFor="is_active" className="text-sm font-bold text-slate-700 dark:text-slate-300 cursor-pointer">This storage location is active</label>
                            </div>
                            ...
                            >
                                {submitting ? <Loader2 className="animate-spin" size={20} /> : editingLocation ? <Edit2 size={20} /> : <Plus size={20} />}
