...
export function FacilityConfigurationModal({ facility, onClose, onUpdate }: FacilityConfigurationModalProps) {
    const { user } = useAuth();
    const role = user?.role?.toUpperCase();
    const canManage = role === 'SUPER_ADMIN' || role === 'SUPER ADMIN' || role === 'FACILITY_ADMIN' || role === 'FACILITY ADMIN';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
            <div className="w-full max-w-3xl bg-white dark:bg-slate-900 rounded-2xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-start shrink-0">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1">
                            {facility.name} <span className="px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 text-[10px] uppercase font-bold">{facility.type}</span>
                        </p>
                    </div>
                ...
                <div className="p-6 overflow-y-auto">
                    {activeTab === 'config' ? (
                        <form onSubmit={handleSubmit(onSubmitConfig)} className="space-y-6 max-w-lg">
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 mb-1">Min Stock Threshold (%)</label>
                                    <p className="text-xs text-slate-500 mb-2">Alert when stock falls below this percentage of max capacity.</p>
                                    <input
                                        type="number"
                                    ...
                                    />
                                    {errors.min_stock_threshold_percentage && <p className="text-red-500 text-xs mt-1">{errors.min_stock_threshold_percentage.message}</p>}
                                </div>

                                <div>
                                    <label className="block text-sm font-bold text-slate-700 mb-1">Expiry Alert Days</label>
                                    <p className="text-xs text-slate-500 mb-2">Days before expiry to start showing warnings.</p>
                                    <input
                                        type="number"
                                        {...register('expiry_alert_days')}
                                        className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-healthcare-primary/20 focus:border-healthcare-primary text-sm font-bold"
                                    />
                                    {errors.expiry_alert_days && <p className="text-red-500 text-xs mt-1">{errors.expiry_alert_days.message}</p>}
                                </div>

                                <div className="space-y-3 pt-2">
                                    <label className="flex items-center gap-3 p-3 border rounded-xl cursor-pointer hover:bg-slate-50 transition-colors">
                                        <input type="checkbox" {...register('departments_enabled')} className="w-4 h-4 text-healthcare-primary rounded" />
                                        <div>
                                            <span className="block text-sm font-bold text-slate-700">Enable Departments</span>
                                            <span className="text-xs text-slate-500">Allow tracking stock across multiple locations</span>
                                        </div>
                                    </label>

                                    <label className="flex items-center gap-3 p-3 border rounded-xl cursor-pointer hover:bg-slate-50 transition-colors">
                                        <input type="checkbox" {...register('controlled_drug_rules_enabled')} className="w-4 h-4 text-healthcare-primary rounded" />
                                        <div>
                                            <span className="block text-sm font-bold text-slate-700">Controlled Drug Rules</span>
                                            <span className="text-xs text-slate-500">Enforce stricter dispensing rules for controlled substances</span>
                                        </div>
                                    </label>
                                </div>
                            </div>
                            ...
                            {canManage && (
                                ...
                                >
                                    {isLoading ? <div className="animate-spin w-5 h-5 border-2 border-white/20 border-t-white rounded-full" /> : <Save size={18} />}
                                    Save Configuration
                                </button>
                            )}
                        </form>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                                <h3 className="text-sm font-bold text-slate-700 mb-3">Add New Department</h3>
                                ...
                                <table className="w-full text-left text-sm">
                                    ...
                                    <tbody className="divide-y">
                                        {departments.map(dept => (
                                            <tr key={dept.id} className="hover:bg-slate-50">
                                                ...
                                                <td className="px-4 py-3 text-slate-500">
                                                    {dept.is_main_store ? <span className="text-teal-600 font-bold text-xs">Main Store</span> : '-'}
                                                </td>
                                        ...
                                        {departments.length === 0 && (
                                            <tr>
                                                <td colSpan={4} className="px-4 py-8 text-center text-slate-500 italic">No departments configured yet.</td>
                                            </tr>
                                        )}
                                    </tbody>
                                </table>
