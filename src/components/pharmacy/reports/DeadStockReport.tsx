...
export function DeadStockReport() {
    ...
    const [overstock, setOverstock] = useState<import('../../../types/pharmacy').OverstockData | null>(null);
    ...
    if (items.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center p-12 bg-white dark:bg-slate-900 rounded-xl border border-gray-100 dark:border-slate-800 text-center">
                ...
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Active Inventory</h3>
        ...
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-gray-100 dark:border-slate-800 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        <div className="text-sm font-medium text-slate-500">Total Value Tied Up</div>
                        <div className="text-2xl font-black text-slate-900 dark:text-white">
                            {new Intl.NumberFormat('sw-TZ', { style: 'currency', currency: 'TZS' }).format(totalValue)}
                        </div>
                    </div>
                </div>
            </div>
                ...
                <table className="w-full text-left text-sm">
                    <thead className="bg-slate-50 dark:bg-slate-800/50">
                        <tr>
                            ...
                            <th className="px-6 py-3 font-semibold text-slate-500 text-right">Last Dispensed</th>
                            <th className="px-6 py-3 font-semibold text-slate-500 text-right">Current Qty</th>
                            <th className="px-6 py-3 font-semibold text-slate-500 text-right">Stock Value</th>
                            <th className="px-6 py-3 font-semibold text-slate-500 text-center">Status</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {items.map((item) => (
                            <tr key={item.medicine_id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                ...
                                <td className="px-6 py-4 text-right text-slate-900 dark:text-white font-mono">
                                    {new Intl.NumberFormat('sw-TZ', { style: 'currency', currency: 'TZS' }).format(Number(item.excess_value))}
                                </td>
