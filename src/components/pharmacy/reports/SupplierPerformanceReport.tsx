...
export function SupplierPerformanceReport() {
    ...
    const [suppliers, setSuppliers] = useState<import('../../../types/pharmacy').SupplierPerformanceItem[]>([]);
                ...
                <table className="w-full text-left text-sm">
                    <thead className="bg-slate-50 dark:bg-slate-800/50">
                        <tr>
                            ...
                            <th className="px-6 py-3 font-semibold text-slate-500 text-center">Total Orders</th>
                            <th className="px-6 py-3 font-semibold text-slate-500 text-center">Avg Lead Time</th>
                            <th className="px-6 py-3 font-semibold text-slate-500 text-right">Fulfillment Rate</th>
                            <th className="px-6 py-3 font-semibold text-slate-500 text-right">On-Time Rate</th>
                            <th className="px-6 py-3 font-semibold text-slate-500 text-center">Rating</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {suppliers.map((item) => (
                            <tr key={item.supplier_id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                ...
                                <td className="px-6 py-4 text-center text-slate-600 dark:text-slate-400">
                                    {Math.round(item.avg_lead_time_days)} days
                                </td>
                                <td className="px-6 py-4 text-right">
                                    <div className="flex items-center justify-end gap-2">
                                        ...
                                        <span className="font-bold text-xs">{item.fulfillment_rate}%</span>
                                    </div>
                                </td>
                                <td className="px-6 py-4 text-right">
                                    <span className={`font-bold ${item.on_time_delivery_rate > 90
                                        ? 'text-emerald-600'
